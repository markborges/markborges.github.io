...
function addBCRepositoryInfo(data) {
    ...
    if (data.download_info) {
        ...
        bcr_header += bcr_separator + '<strong>Download</strong>: <a href="' + first_download[2] + '" onClick="ga.getAll()[0].send(\'event\', \'APPs\', \'Download\', this.href);">' + first_download[1] + '</a>' + downloadState(first_download[0]);
        bcr_separator = ' | ';
    }
    ...
    if (data.github_discussion) {
        bcr_footer += '<p style="margin:0px;padding:5px 0px">Comments? Share them on <a href="' + data.github_discussion + '"><i class="fab fa-github fa-fw"></i> GitHub</a></p>';
