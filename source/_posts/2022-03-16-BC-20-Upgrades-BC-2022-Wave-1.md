...
Enabling this new feature called **Check documents and journals while you work** will allow users to get "early, unobstrusive visual indicators that there is a problem" on Documents and Journal Lines, as you work on them.
...
Because we have all been waiting for this, for years! 🍾🍾🍾🍾🍾
...
I have to confess this was a little puzzling to me at first, because **Deposits** have been a thing in the North American version of Navision and Business Central for quite a while, that I even forgot this was **only** available on the NA version. So, why would this be a "new" feature? 😵
...
Notes for **NA users**:
* 🟡 Review this functionality when it's released, as there will be a new flag on the Deposit Header called **Post as Lump Sum**
* 🔺 Data will **NOT** be migrated from NA Deposit tables to the new Bank Deposit tables. This means that when you start using the new Bank Deposits, you will have history sitting in 2 different parts of the system, unfortunately.
...
Microsoft developed an extension that will help users (mostly partners) generate demo data for the Manufacturing Module. It seems they are making a bit of a big thing out of it, so that they can replicate this demo data pattern to other areas of the system, including to partners who write their own verticals.

At this point, it seems the **Contoso Coffee Demo Dataset** will come as an extension available throught App Source, and also available as Source Code on the Product Media (DVD). If you're interested on that, keep an eye on those 2 channels.
...
* **Not only Filter** the Demand View, **but also enter** Forecast **per Variant**
  - This will show the same item multiple times (as multiple rows), one for each variant 😲
* **Enter Forecast per Location**
  - This will show the same item multiple times (as multiple rows), one for each location!! And it **CAN** be used in conjunction with the one for Variants 😲😲😲
...
There are quite a few other improvements coming on this release that I did not add to my list, either because they were too technical, or because they were just minor to me. Therefore, please make sure to review the original Microsoft [Release Plan](https://docs.microsoft.com/en-us/dynamics365-release-plan/2022wave1/smb/dynamics365-business-central/planned-features) if you want to know every single new thing coming on 🟦**BC 20**🟦. 

**P.S**.: **Microsoft** is improving the list of planned features during this month. So, I have a [**Part 2**](/2022/03/24/BC-20-Upgrades-Part-2) post for this. 🍻